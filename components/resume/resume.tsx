...
const Resume: React.FC = () => {
  return (
    <motion.div
      ...
      className="min-h-[80vh] flex items-center justify-center py-12 xl:py-0 min-w-[75rem]"
    >
                ...
                <ScrollArea className="h-[400px]">
                  <ul className="grid grid-cols-1 lg:grid-cols-2 gap-[30px]">
                    {education.items.map((item, index) => (
                      <Card key={index} title={item.degree} subtitle={item.institution} duration={item.duration} />
                    ))}
                  </ul>
                </ScrollArea>
              </Section>
...
const SkillCard: React.FC<{ icon: JSX.Element; name: string }> = ({ icon, name }) => (
